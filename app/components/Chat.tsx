...
export default function Chat() {
  ...
  // Render loading indicator without animations
    ...
    <div className="flex flex-col w-full max-w-sm rounded-2xl overflow-hidden shadow-lg h-[600px]">
      ...
      {mounted ? (
        ...
        <div className="p-3 border-t border-zinc-800 bg-zinc-900 flex-shrink-0">
          {/* Static version of the input area */}
          <div className="flex overflow-x-auto gap-2 pb-3 scrollbar-hide">
            {PROMPT_TEMPLATES.map((prompt, index) => (
              <button
                key={index}
                disabled
                className="flex-shrink-0 px-3 py-1.5 bg-zinc-800 border border-zinc-700 text-white text-sm rounded-lg opacity-50"
              >
                {prompt}
              </button>
            ))}
          ...
          <div className="relative flex-1">
            <input
              type="text"
              disabled
              className="w-full px-3 py-1.5 pr-10 bg-zinc-800 text-white text-sm border border-zinc-700 rounded-lg opacity-50"
              placeholder="How can I help you?"
            />
          </div>
        </div>
